...
var Cloud = new Class({
	...
	*/
	...
	switchChannel: function(c) {
		...
		$$('.object1, .object2, .object3, .object4, .object_to, .daggerline').destroy();
		/* Change Channel */
		this.actualChannel = this._channels[c];
		/* Redraw */
		this.drawObjects();
	},
	...
	drawObjects: function() {
		...
		for( var k in this._objectstack[this.actualChannel] ) {
			if (k<=this._objectstack[this.actualChannel].length) {
				this._objectstack[this.actualChannel][k]['fromid'] = this.addObject(this._objectstack[this.actualChannel][k]['from'][0],this._objectstack[this.actualChannel][k]['from'][1],this._objectstack[this.actualChannel][k].key,this._objectstack[this.actualChannel][k].legends,this._objectstack[this.actualChannel][k].type);
				$(this._objectstack[this.actualChannel][k]['fromid']).k = k;

				if (typeof(this._objectstack[this.actualChannel][k]['to'])=="object") {
					this._objectstack[this.actualChannel][k]['toid'] = this.addObject(this._objectstack[this.actualChannel][k]['to'][0],this._objectstack[this.actualChannel][k]['to'][1],this._objectstack[this.actualChannel][k].key,this._objectstack[this.actualChannel][k].legends);
	...
	*/
	...
	addObject: function(l,t,v,leg,s) {
		if (s==null) s = false;
		...
		new_e.legendhorizontal = leg[0];
		new_e.legendvertical = leg[1];
	...
	initobject: function(elm,slave) {
		...
		elm.makeDraggable({
			...
			onCancel: function(elm){
				...
				// Delete word - for slaves only
				if (slave && !this._drawstate) {
	...
	*/
	...
	loadJSON: function(data) {
		...
		for( var d in this._datapool ) {
			...
			for(var k in this._datapool[d])  if (this._datapool[d].hasOwnProperty(k))  {
				if (this._datapool[d][k].from == null)
					this._datapool[d][k].from = [10,10]
	...
	*/
	...
	updateJSON: function () {
		...
		this.savetimeout = this._updateJSON.delay(500, this);	
	},
...
//		console.log(this._objectstack);
		...
		this._datapool = this._objectstack;	
	...
	*/
	...
	addSlave: function(elm) {
		this._objectstack[this.actualChannel][elm.k]['toid'] = this.addObject(50,50,this._objectstack[this.actualChannel][elm.k].key,this._objectstack[this.actualChannel][elm.k].legends);
...
//		this._currentcanvas.context.lineWidth = 20;
		...
		for (var i=1; i < dx/fx; i+=5) {
			var sx = lfrx+(i*fx);
			var sy = lfry+(i*fy);
			this._currentcanvas.context.lineWidth = 20 - (18 / (dx/fx) * (i - 1));

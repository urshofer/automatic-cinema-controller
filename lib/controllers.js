...
.controller('AppCtrl', function($rootScope, $scope, $ionicPopup, NetService, md5, $q,  $state) {
	...
	$scope.changePreroll = function(preroll) {
		$scope.preroll = preroll;
		if ($scope.prerolltimeout) {
			clearTimeout($scope.prerolltimeout);
		}
		$scope.prerolltimeout = setTimeout($scope.storePreroll, 500);
	}
		
	$scope.storePreroll = function() {
		console.log("Stored Preroll: " + $scope.preroll);
		$scope.send({preroll:$scope.preroll}, "preroll");
	}	
	...
	$scope.loadconfig = function() {
		...
		NetService.loadconfig($scope.auth_token).then(function(data){
			...
			$scope.preroll = $scope.configuration.preroll;
	...
	$ionicModal.fromTemplateUrl('addcontent.html', {
	  ...
	  $scope.content_modal = data
	    ...
	    $scope.save = function(type) {
			console.log($scope.active_modal.data);
			NetService.store($scope.active_modal.data, ($scope.active_modal.data.update?"update":"add")+type, $scope.auth_token).then(function(data){
				if (data.Error) {
					$ionicPopup.alert({title: 'Error ' + data.Error.Code, template: data.Error.Message});
				}
  				$scope.reload();
		...
		$scope.editchannel = function(channel) {
			$scope.active_modal = $scope.channel_modal
			$scope.active_modal.data = channel;
			$scope.active_modal.data.update = true;			
			$scope.active_modal.show();			
		}
...
.controller('ContentCtrl', function($scope, $stateParams, $q, $ionicPopup, NetService) {
	...
	/* Auto Reloader Stuff */
	$scope.automaticrefresh = false;				
	$scope.ontologyReload = function() {
		NetService.load("content", $scope.auth_token).then(function(data){
			...
			$scope.data = data;
			$scope.ontology.options.data = $scope.data;
			$scope.ontology.loadJSON($scope.ontology.options.data);
			$scope.ontology.redraw()
		});
	}
	$scope.refreshData = function() {
		if ($scope.refreshinterval) {
			...
			$scope.automaticrefresh = false;
			$scope.refreshinterval  = false;
		}
		else {
			$scope.refreshinterval = setInterval($scope.ontologyReload, 3000);
			$scope.automaticrefresh = true;			
		}
	}

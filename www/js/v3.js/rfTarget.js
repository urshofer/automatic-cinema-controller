...
var Target = new Class({
	...
	*/
	...
	initialize: function(container, options) {  
		...
		this.loadJSON(this.options.json);
	...
	redraw: function() {
		console.log("delete");
		$$('div.targetmatrix div.object, div.targetmatrix div.target').destroy();
		this._keywordstack 		= [];
		this._targetstack 		= [];
	...
	*/
	...
	initobject: function(elm) {
		...
		if (this._targetstack[dim][2]==1) elm.addClass('active');
		...
		elm.makeDraggable({
			...
			onCancel: function(elm){
				...
				this._targetstack[dim][2] = (this._targetstack[dim][2])?(0):(1);
				if (this._targetstack[dim][2]==1) elm.addClass('active');
				if (this._targetstack[dim][2]==0) elm.removeClass('active');
				this.updateJSON();
		    }.bind(this),
